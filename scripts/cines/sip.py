...
def get_srusrw_url() :
	url_title = urllib.quote(re.sub(r'([^\s\w\'-]|_)+', '', get_title().lower().encode('utf-8').replace('é', 'e').replace('ç', 'c').replace('è', 'e').replace('ê', 'e').replace('ń', 'n').replace('œ', 'oe').replace('á', 'a').replace('ä', 'a').replace('ë', 'e')), safe='')
	url_creator = urllib.quote(re.sub(r'([^\s\w\'-]|_)+', '', get_creator().lower().encode('utf-8').replace('é', 'e').replace('ç', 'c').replace('è', 'e').replace('ê', 'e').replace('ń', 'n').replace('œ', 'oe').replace('á', 'a').replace('ä', 'a').replace('ë', 'e')), safe='')
...
# BeQuali Filter
# Return the date of the most recent document
# Based on the EAD part, generated by Ligeo
def latest_date_filter(values) :
	# Build an array with only the dates
	dates = []
	for value in values :
...
# BeQuali Filter
# Return the date of the oldest document
# Based on the EAD part, generated by Ligeo
def oldest_date_filter(values) :
	# Build an array with only the dates
	dates = []
	for value in values :
		dates.append(value.text)
...
# Translate a key based on a dict
def translate_bis_filter(values) :
	return [translate_bis_dict[values[0].text].decode('UTF-8')]
